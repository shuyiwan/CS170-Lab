...
cmd_free(command_t *cmd)
{
	...
	/* Your code here. */
	// free all the used cmd arguemant and redirect filename, set pointer to be NULL
	...
	// free subshell and next pointer
...
cmd_parse(parsestate_t *parsestate)
{
	...
	while (1) {
		...
		case TOK_2_GREATER_THAN: {
            ...
            /* Your code here. */
		case TOK_OPEN_PAREN:
			// check if one cmd only has one subshell
			if (cmd->subshell != NULL || i > 0)
				goto error;

			else // recursively go to cmd_line_parse
